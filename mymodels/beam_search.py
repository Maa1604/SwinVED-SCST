...
from transformers import Constraint, BeamScorer, BeamSearchScorer
...
def prepare_inputs_for_generation(self, input_ids, past=None, attention_mask=None, input_embeds = None, **model_kwargs):
...
#             # Use `input_embeds` instead of `input_ids` if provided
#             inputs = {"inputs_embeds": input_embeds}
#         else:
#             inputs = {"input_ids": input_ids}
        
#         # Add attention_mask and past if present
#         inputs["attention_mask"] = attention_mask if attention_mask is not None else input_ids.new_ones(input_ids.shape)
#         if past is not None:
#             inputs["past_key_values"] = past
        
#         # Pass other model-specific arguments
