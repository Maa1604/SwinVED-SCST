...
class SCST(nn.Module):
    ...
    def forward_greedy(self, questions_ids, questions_mask, answers_ids, answers_mask, images, model, images_mask=None):
        ...
        # usar answers_ids y answers_mask para sacar solo los ids de answer sin el padding

        # Calculate Greedy Rewards
        greedy_input_ids = out.sequences.detach().data

        # print(f"esto es greedy_input_ids {greedy_input_ids}")
        


        # Find the first occurrence of 0 in each row and slice accordingly (funciona aunque haya 2 ceros)
        if (greedy_input_ids == self.bos_token_id).any():  # Check if there are any zeros in the tensor
            row, col = (greedy_input_ids == self.bos_token_id).nonzero(as_tuple=True)  # Get row and column indices of zeros
            hyp = greedy_input_ids[:, col[0] + 1:]  # Slice from the first zero in the first row (preserving 2D)
        else:
            hyp = greedy_input_ids  # If no zeros, keep the tensor unchanged
        ...
        # print(answers_ids)
        # print(answers_mask)
        # print(f"esto es hyp {hyp}")

        questions_ids_trimmed = answers_ids[answers_mask.bool()]
        # print(f"esto es answers_ids {answers_ids}")
        # print(f"esto es answers_mask {answers_mask}")
        # print(f"esto es questions_ids_trimmed {questions_ids_trimmed}")

        question_p = torch.cat([
            # torch.tensor([self.bos_token_id], device=answers_ids.device),  # BOS token
            questions_ids_trimmed,
            torch.tensor([self.eos_token_id], device=answers_ids.device)   # EOS token
        ])

        ref = question_p.unsqueeze(0)

        # print(ref)
        # print(f"esto es ref {ref}")


        # print("greddy")
        reward_greedy, generated_reports_list, reference_reports_list = self.get_reward(hyp, ref, model.tokenizer) #question[BOS]Answer[EOS]

        # print(f"esto es greedy generated_reports_list {generated_reports_list}")
        # print(f"esto es greedy reference_reports_list {reference_reports_list}")
        # print(f"esto es reward_greedy {reward_greedy}")
    ...
    def forward_sampling(self, questions_ids, questions_mask, answers_ids, answers_mask, reward_greedy, images, model, labels, images_mask=None):
        ...
        # Calculate NLL
        if self.use_nll:
            decoder_out = model(questions_ids, questions_mask, answers_ids, answers_mask,
                                        images, images_mask=images_mask, labels=labels)
        ...
        # print(f"2,5: {out}")
        # print(f"2,5: {out.size}")
        # print(f"3: {out.sequences}")
        # print("Shape of sequences:", out.sequences.shape)

        # print("Number of steps (tokens generated):", len(out.scores))
        end_token_id = 10  # token that ends the first sentence
        sampled_ids = []
        cutoff_index = None  # for truncating scores later
        for row in out.sequences:
            # Remove BOS token if present
            if (row == self.bos_token_id).any():
                row = row[(row == self.bos_token_id).nonzero(as_tuple=True)[0][0] + 1:].contiguous()
            else:
                row = row.contiguous()

            # Cut at first token with ID == 10 (inclusive)
            if (row == end_token_id).any():
                idx = (row == end_token_id).nonzero(as_tuple=True)[0][0].item() + 1
                row = row[:idx]
                cutoff_index = idx
            else:
                cutoff_index = row.size(0)
        ...
        # print(sampled_ids.data)

        questions_ids_trimmed = answers_ids[answers_mask.bool()]

        question_p = torch.cat([
            # torch.tensor([self.bos_token_id], device=answers_ids.device),
            questions_ids_trimmed,
            torch.tensor([self.eos_token_id], device=answers_ids.device)   # EOS token
        ])
        ...
        # print(ref)
        # print(f"input_ids_questions {ref}")
        # exit()

        # Calculate Sampling Rewards  
        # print("SAMPLING")
        reward_sampling, generated_reports_list, _ = self.get_reward(sampled_ids.data, ref, model.tokenizer)

        # exit()

        # print(f"esto es sampling generated_reports_list {generated_reports_list}")
        # print(f"esto es reward_sampling {reward_sampling}")
    ...
    def get_reward(self, generated_reports_input_ids, reference_reports_input_ids, tokenizer):
        ...
        for gen_rep, ref_rep in zip(generated_reports_input_ids, reference_reports_input_ids):
            ...
            # print(f"generated: {tokenizer.decode(gen_rep, skip_special_tokens=True, clean_up_tokenization_spaces=False)}")

            reference_reports_list.append(tokenizer.decode(ref_rep, skip_special_tokens=True, clean_up_tokenization_spaces=False))   
            # print(f"reference: {tokenizer.decode(ref_rep, skip_special_tokens=True, clean_up_tokenization_spaces=False)}")    
            # 
            # print(f"generated_reports_list:\n {generated_reports_list}")
            # print(f"reference_reports_list:\n {reference_reports_list}") 
            
        reward_list = []
        for scorer, scorer_index in zip(self.scorers, self.scorers_index):
            #print(scorer)
            try:
                # if isinstance(scorer, MeteorScorer):
                #     score_out = scorer.compute_score(generated_reports_list, reference_reports_list)
                # elif isinstance(scorer, Cider):
                #     gts = {i: [ref] for i, ref in enumerate(reference_reports_list)}
                #     res = {i: [hyp] for i, hyp in enumerate(generated_reports_list)}
                #     score_out = scorer.compute_score(gts=gts, res=res)[0]
                # elif isinstance(scorer, Rouge):
                #     score_out = scorer(refs=reference_reports_list, hyps=generated_reports_list)[0]
                # elif isinstance(scorer, Bleu):
                score_out = scorer(reference_reports_list, generated_reports_list)
                # print(f"{scorer.__class__.__name__}: {score_out}")
            except:
